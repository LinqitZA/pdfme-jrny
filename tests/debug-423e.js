...
async function main() {
  const r1 = await fetch('http://localhost:3001/api/pdfme/templates', {
    ...
    body: JSON.stringify({ name: 'Debug423e', type: 'invoice', schema: { pages: [{ elements: [{ name: 'lineItems', type: 'lineItemsTable', position: { x: 10, y: 30 }, width: 190, height: 100, showHeader: true, columns: [{ key: 'desc', header: 'Desc', width: 80 }, { key: 'qty', header: 'Qty', width: 30 }, { key: 'amt', header: 'Amount', width: 80 }] }] }] } }),
  });
  ...
  const r3 = await fetch('http://localhost:3001/api/pdfme/render/now', {
    method: 'POST', headers: h,
    body: JSON.stringify({ templateId: tid, inputs: { lineItems: JSON.stringify([['Widget A', '5', '250.00']]) } }),
  });
  const d3 = await r3.text();
  process.stdout.write('Render: ' + r3.status + ' ' + d3.substring(0, 500) + '\n');
