...
async function main() {
  const r1 = await fetch('http://localhost:3001/api/pdfme/templates', {
    ...
    body: JSON.stringify({ name: 'Debug423c', type: 'invoice', schema: { schemas: [[{ name: 'lineItems', type: 'lineItemsTable', position: { x: 10, y: 30 }, width: 190, height: 100, showHeader: true, columns: [{ key: 'desc', header: 'Desc', width: 80 }] }]], basePdf: 'BLANK_PDF' } }),
  });
  ...
  // Publish directly
  const r2 = await fetch('http://localhost:3001/api/pdfme/templates/' + tid + '/publish', { method: 'POST', headers: h });
  const d2 = await r2.text();
  process.stdout.write('Publish: ' + r2.status + ' ' + d2.substring(0, 500) + '\n');
