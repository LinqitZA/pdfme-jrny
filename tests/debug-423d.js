...
async function main() {
  const r1 = await fetch('http://localhost:3001/api/pdfme/templates', {
    ...
    body: JSON.stringify({ name: 'Debug423d', type: 'invoice', schema: { pages: [{ elements: [{ name: 'lineItems', type: 'lineItemsTable', position: { x: 10, y: 30 }, width: 190, height: 100, showHeader: true, columns: [{ key: 'desc', header: 'Desc', width: 80 }] }] }] } }),
  });
