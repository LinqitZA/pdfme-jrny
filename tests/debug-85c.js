...
const token = jwt.sign(
  { sub: 'u1', orgId: 'org-85c', role: 'admin', permissions: ['template:read','template:write','document:read','document:write','render:trigger'] },
  SECRET,
  { expiresIn: '1h' }
);
...
async function run() {
  ...
  fs.writeFileSync('/tmp/debug-85c.txt', 'render: ' + render.status + ' ' + render.body.substring(0, 1000));
