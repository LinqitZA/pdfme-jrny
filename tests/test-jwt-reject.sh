...
curl -s -w '\nHTTP_STATUS: %{http_code}\n' -X POST \
  -H "Content-Type: application/json" \
  -d '{"templateId":"abc"}' \
  http://localhost:3000/api/pdfme/render/now

echo ""
echo "--- Test 4: Health endpoint still public (expect 200) ---"
curl -s -w '\nHTTP_STATUS: %{http_code}\n' http://localhost:3000/api/pdfme/health

echo ""
echo "--- Test 5: Verify error envelope format ---"
curl -s http://localhost:3000/api/pdfme/templates | python3 -m json.tool 2>/dev/null || curl -s http://localhost:3000/api/pdfme/templates
