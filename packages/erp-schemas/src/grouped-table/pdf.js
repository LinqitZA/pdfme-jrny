...
async function pdfRender(arg) {
    ...
    const height = schema.height || 150;
    const headerStyle = schema.headerStyle || {};
    // Parse body data
    let bodyRows = [];
    if (value) {
        try {
    ...
    const headerFontSize = headerStyle.fontSize || 9;
    ...
    if (columns.length > 0) {
        ...
        const bgColor = hexToRgb(headerStyle.backgroundColor || '#e0e0e0');
        page.drawRectangle({
            x,
