...
exports.rectangle = exports.SHADOW_DEFAULTS = exports.RECTANGLE_DEFAULTS = void 0;
exports.parseHexColorWithAlpha = parseHexColorWithAlpha;
exports.toUpstreamRectangleSchema = toUpstreamRectangleSchema;
exports.resolveRectangles = resolveRectangles;
exports.applyRectangleShadows = applyRectangleShadows;
exports.RECTANGLE_DEFAULTS = {
    type: 'rectangle',
    position: { x: 0, y: 0 },
    width: 80,
    height: 50,
 ...
 */
function toUpstreamRectangleSchema(element) {
    return {
        type: 'rectangle',
        name: element.name,
 ...
 */
function resolveRectangles(template, _inputs) {
    const shadowElements = [];
    if (!Array.isArray(template.schemas)) {
        return { template, shadowElements };
    }
    const newSchemas = template.schemas.map((page, pageIndex) => {
        ...
        return page.map((field) => {
            ...
            if (!isErpRect)
                return field; // leave upstream rectangles alone
            const rect = f;
 ...
 */
async function applyRectangleShadows(pdfBytes, shadowElements) {
    ...
    for (const elem of shadowElements) {
        if (elem.pageIndex >= pages.length)
            continue;
...
exports.rectangle = {
    ...
    RECTANGLE_DEFAULTS: exports.RECTANGLE_DEFAULTS,
    SHADOW_DEFAULTS: exports.SHADOW_DEFAULTS,
};
