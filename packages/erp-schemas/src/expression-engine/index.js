...
class ExpressionEngine {
    ...
    constructor(options = {}) {
        ...
        this.parser = new expr_eval_fork_1.Parser({
            operators: {
     ...
     */
    static BLOCKED_KEYWORDS = new Set([
     ...
     */
    assertNoBlockedKeywords(expression) {
        for (const keyword of ExpressionEngine.BLOCKED_KEYWORDS) {
            ...
            if (regex.test(expression)) {
                throw new Error(`Sandbox violation: "${keyword}" is not allowed in expressions. ` +
     ...
     */
    resolveFieldReferences(expression, context) {
        const flatContext = {};
     ...
     */
    static toStr(val) {
        if (val instanceof NullSentinel || val === null || val === undefined)
            return '';
        return String(val);
    }
    registerStringFunctions() {
        ...
        this.parser.functions.TRIM = (str) => {
        ...
        this.parser.functions.PADLEFT = (str, length, padChar) => {
            const s = toStr(str);
        ...
        this.parser.functions.SUBSTITUTE = (str, search, replacement) => {
            const s = toStr(str);
            const searchStr = toStr(search);
            const replaceStr = toStr(replacement);
            if (searchStr === '')
        ...
        this.parser.functions.SPLIT = (str, delimiter, index) => {
            const s = toStr(str);
        ...
        this.parser.functions.FIND = (searchStr, withinStr) => {
    ...
    registerConditionalFunctions() {
        ...
        this.parser.functions.IF = (condition, trueVal, falseVal) => {
            return condition ? trueVal : falseVal;
        };
        // AND(a, b, ...) - logical AND
        this.parser.functions.AND = (...args) => {
            return args.every((a) => Boolean(a));
        };
        // OR(a, b, ...) - logical OR
        this.parser.functions.OR = (...args) => {
            return args.some((a) => Boolean(a));
        };
        // NOT(value) - logical NOT
        this.parser.functions.NOT = (value) => {
            return !value;
        };
        ...
        this.parser.functions.SWITCH = (...args) => {
            if (args.length < 3)
                return '';
            const expr = args[0];
    ...
    registerDateFunctions() {
        ...
        this.parser.functions.FORMAT = (value, pattern) => {
    ...
    registerNumericFunctions() {
        ...
        this.parser.functions.ROUND = (value, decimals = 0) => {
            if (typeof value !== 'number')
                value = Number(value);
            if (isNaN(value))
                return 0;
        ...
        this.parser.functions.ABS = (value) => {
            if (typeof value !== 'number')
                value = Number(value);
            return Math.abs(value);
        };
        // FLOOR(value) - round down to nearest integer
        this.parser.functions.FLOOR = (value) => {
            if (typeof value !== 'number')
                value = Number(value);
            if (isNaN(value))
                return 0;
        ...
        this.parser.functions.CEIL = (value) => {
            if (typeof value !== 'number')
                value = Number(value);
            if (isNaN(value))
                return 0;
        ...
        this.parser.functions.MIN = (...args) => {
            const nums = args.map(a => {
                if (a instanceof NullSentinel || a === null || a === undefined)
                    return 0;
                const n = Number(a);
                return isNaN(n) ? 0 : n;
            });
    ...
    registerLocaleAwareFunctions() {
        ...
        this.parser.functions.FORMAT_DATE = (value) => {
            const date = this.toDate(value);
            try {
        ...
        this.parser.functions.FORMAT_NUMBER = (value) => {
            if (typeof value !== 'number')
                value = Number(value);
            try {
