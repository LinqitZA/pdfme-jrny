...
const getFontProp = ({ value, fontKitFont, schema, colorType, }) => {
    ...
    if (schema.dynamicFontSize) {
        fontSize = (0, helper_js_1.calculateDynamicFontSize)({ textSchema: schema, fontKitFont, value });
    }
    else if (schema.textOverflow === 'shrinkToFit') {
        // shrinkToFit: use dynamic font sizing with min=6pt and max=current fontSize
        const baseFontSize = schema.fontSize ?? constants_js_1.DEFAULT_FONT_SIZE;
        const shrinkSchema = {
            ...schema,
            dynamicFontSize: {
                min: constants_js_1.SHRINK_TO_FIT_MIN_FONT_SIZE,
                max: baseFontSize,
                fit: constants_js_1.DYNAMIC_FIT_VERTICAL,
            },
        };
        fontSize = (0, helper_js_1.calculateDynamicFontSize)({
        ...
        if (fontSize < constants_js_1.SHRINK_TO_FIT_MIN_FONT_SIZE) {
            fontSize = constants_js_1.SHRINK_TO_FIT_MIN_FONT_SIZE;
        }
    }
    else {
        fontSize = schema.fontSize ?? constants_js_1.DEFAULT_FONT_SIZE;
    }
...
const pdfRender = async (arg) => {
    ...
    let lines = (0, helper_js_1.splitTextToSize)({
        value,
        characterSpacing,
        fontSize,
        fontKitFont,
    ...
    if (textOverflow === 'truncate') {
        ...
        const ellipsis = '...';
        const ellipsisWidth = (0, helper_js_1.widthOfTextAtSize)(ellipsis, fontKitFont, fontSize, characterSpacing);
        if (lines.length > maxLines) {
            lines = lines.slice(0, maxLines);
            // Add ellipsis to the last visible line
            let lastLine = lines[maxLines - 1].replace('\n', '');
            const lastLineWidth = (0, helper_js_1.widthOfTextAtSize)(lastLine, fontKitFont, fontSize, characterSpacing);
            if (lastLineWidth + ellipsisWidth > width) {
                // Need to trim the last line to fit ellipsis
                while (lastLine.length > 0 && (0, helper_js_1.widthOfTextAtSize)(lastLine + ellipsis, fontKitFont, fontSize, characterSpacing) > width) {
            ...
            lines = lines.map((line, idx) => {
                ...
                const lineWidth = (0, helper_js_1.widthOfTextAtSize)(trimmedLine, fontKitFont, fontSize, characterSpacing);
                if (lineWidth > width && trimmedLine.length > 3) {
                    let truncated = trimmedLine;
                    while (truncated.length > 0 && (0, helper_js_1.widthOfTextAtSize)(truncated + ellipsis, fontKitFont, fontSize, characterSpacing) > width) {
