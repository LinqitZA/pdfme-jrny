...
export default function React19CompatTests() {
  const [results, setResults] = useState<TestResult[]>([
    // react-moveable tests
  ...
  // Capture React 19 specific console warnings
  useEffect(() => {
...
function MoveableTest({
  ...
  // Mark mount as pass after hydration
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        ...
        if (moveableEl) {
          setMounted(true);
          onResult('react-moveable', 'Mount & render', 'pass', 'Component mounted with React 19');
        } else {
          onResult(
            'react-moveable',
            'Mount & render',
            'fail',
            'Component did not render DOM elements',
          );
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (resizeCount > 0) {
      onResult(
        ...
        'Resize operation',
        'pass',
        `${resizeCount} resize event(s) fired`,
      );
    }
  ...
  useEffect(() => {
    if (rotateCount > 0) {
      onResult(
        ...
        'Rotate operation',
        'pass',
        `${rotateCount} rotate event(s) fired`,
      );
    }
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: '#666', marginBottom: 8 }}>
        Drag, resize, or rotate the blue box below. Status: {mounted ? 'Mounted' : 'Mounting...'}
      </p>
      <div
        ref={containerRef}
...
function SelectoTest({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        // Selecto renders a hidden container element
        setMounted(true);
        onResult(
          'react-selecto',
          'Mount & render',
          'pass',
          'Selecto component mounted, ready for drag-select',
        );
      } catch (err) {
        onResult('react-selecto', 'Mount & render', 'fail', String(err));
      }
    }, 1000);
    return () => clearTimeout(timer);
  }, [onResult]);
  ...
  useEffect(() => {
    if (dragStartCount > 0) {
      onResult(
        ...
        'pass',
        `${dragStartCount} dragStart event(s)`,
      );
    }
  ...
  useEffect(() => {
    if (selectCount > 0) {
      onResult(
        ...
        'onSelect callback',
        'pass',
        `${selectCount} select event(s), items: ${selectedItems.join(', ')}`,
      );
    }
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: '#666', marginBottom: 8 }}>
        Click or drag to select the colored boxes below.
      </p>
      <div
        ref={containerRef}
      ...
      >
        ...
        {mounted && containerRef.current && (
          <SelectoComponent
            ...
            onSelect={(e) => {
              const selected = (e.selected as HTMLElement[]).map(
                (el) => el.dataset.index || 'unknown',
              );
              setSelectedItems(selected);
              setSelectCount((c) => c + 1);

              // Visual feedback
              (e.added as HTMLElement[]).forEach((el) => {
                el.style.outline = '3px solid #000';
              });
              (e.removed as HTMLElement[]).forEach((el) => {
      ...
      <div style={{ fontSize: 12, marginTop: 4, color: '#999' }}>
        Events: dragStart={dragStartCount}, select={selectCount} | Selected:{' '}
...
function GuidesTest({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        // Check if the guides component rendered canvases
        const canvases = document.querySelectorAll('#guides-container canvas');
        const hCanvas = canvases.length >= 1;
        const vCanvas = canvases.length >= 2;

        if (hCanvas) {
          onResult(
            '@scena/react-guides',
            'Mount horizontal ruler',
            'pass',
            'Horizontal ruler canvas rendered',
          );
        } else {
          onResult(
            '@scena/react-guides',
            'Mount horizontal ruler',
            'fail',
            'No horizontal ruler canvas found',
          );
        }
        ...
        if (vCanvas) {
          onResult(
            '@scena/react-guides',
            'Mount vertical ruler',
            'pass',
            'Vertical ruler canvas rendered',
          );
        } else {
          onResult(
            '@scena/react-guides',
            'Mount vertical ruler',
            'fail',
            `Only ${canvases.length} canvas(es) found`,
          );
        }

        // Test scroll/zoom response via ref methods
        if (hRef.current && vRef.current) {
          try {
            ...
            onResult(
              ...
              'pass',
              'scroll() and resize() methods callable via ref',
            );
          } catch (refErr) {
            onResult(
              '@scena/react-guides',
              'Scroll/zoom response',
              'fail',
              `Ref method error: ${String(refErr)}`,
            );
          }
        } else {
          onResult(
            ...
            'Scroll/zoom response',
            'fail',
            'Refs not attached to Guides components',
          );
        }
      } catch (err) {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: '#666', marginBottom: 8 }}>
        Horizontal and vertical rulers should render below with tick marks.
      </p>
      <div
        id="guides-container"
        style={{
      ...
      >
        ...
        >
          Canvas area (rulers should show markings above and left)
        </div>
      </div>
    </div>
  );
